...
groupBaseline_L<-function(baseline_L, groupBy, nproc=1) {
  ret_class<-new("Baseline",description=baseline_L@description, db=baseline_L@db, 
                 regionDefinition=baseline_L@regionDefinition[[2]], testStatistic=baseline_L@testStatistic,
...
# 8. Generating region definition list for all clones in db:
clones_regions_list<-make_regions_dict(all_clones_merged_df)

#10. Calculating "consensus" sequence through "collapasClones" on all clones, and drawing PDF: 
...
# adding a columns with same value - for the groupBaseline pdf below:
collapsed_db_baseline@db$SAME<-"same"
collapsed_db_group_baseline<-groupBaseline_L(baseline=collapsed_db_baseline,groupBy = "SAME")

collapsed_db_group<-groupBaseline(collapsed_db_baseline,groupBy = CLONE)

collapsed_db_baseline@db$SAME<-"same_val"
collapsed_db_group_baseline_ALL<-groupBaseline(baseline = collapsed_db_baseline_ALL, 
                                               groupBy = c("SAME"))
plotBaselineDensity(collapsed_db_group_baseline_ALL,idColumn = "SAME",sigmaLimits = c(-0.2,0.2),
                    title="PDF of collapsed clones consensus sequences")


#10. adding to db (before collapsed) - the expected and obsereved mutation columns - for all regions.
